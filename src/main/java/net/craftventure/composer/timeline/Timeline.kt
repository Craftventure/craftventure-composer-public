package net.craftventure.composer.timeline

import net.craftventure.composer.extension.orIfNan
...
//        }
    ...
    fun getFrameTimeBetween(startSeconds: Double, endSeconds: Double): Long? {
        ...
        for (frame in keyframes) {
            if (frame.at in startMillis..endMillis) {
                return frame.at
            }
        }
        return null
            ...
            for ((index, keyframe) in keyframes.withIndex()) {
                ...
                if (previousKeyframe.at <= millis && keyframe.at >= millis) {
                    ...
                    if (previousKeyframe.outEasing == KeyFrameEasing.NONE && keyframe.inEasing == KeyFrameEasing.NONE) {
                        return safe(previousKeyframe.keyValue.toDouble() + ((keyframe.keyValue.toDouble() - previousKeyframe.keyValue.toDouble()) * percentage)).toDouble().orIfNan(0.0)
                    } else {
                        ...
                        val tPreviousValue = (previousKeyframe.keyValue.toDouble() * (1 - tPrevious) + keyframe.keyValue.toDouble() * tPrevious)
                        val tNextValue = (previousKeyframe.keyValue.toDouble() * (1 - tNext) + keyframe.keyValue.toDouble() * tNext)
